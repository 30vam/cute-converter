...
TorqueConverter::TorqueConverter(QWidget *parent)
...
void TorqueConverter::convertValues(int unitIndex, QString valueString)
{
    ...
    else
    {
        ...
        double toNewtonMetreCoefficient = m_unitList.at(unitIndex).second;
        double inputToNewtonMetre = inputDouble / toNewtonMetreCoefficient;

        for(int i = 0; i < m_lineEditList.count(); i++)
        {
            //This condition makes it so the LineEdit which is correctly being edited is not affected by the loop
            if (i != unitIndex) {
                double convertedValue = inputToNewtonMetre * m_unitList.at(i).second;
